...
const fieldClass =
  "mt-1.5 w-full rounded-2xl border border-cream-dark bg-white px-4 py-3 text-base text-text shadow-sm outline-none transition-[border-color,box-shadow] placeholder:text-text-muted/60 focus:border-teal focus:ring-2 focus:ring-teal/25";
...
export function BetaLandingForm() {
  ...
  if (ok) {
    return (
      <div
        className="animate-soft-pop surface-raised px-6 py-8 text-center"
        role="status"
      >
  ...
  return (
    <form
      onSubmit={onSubmit}
      className="animate-fade-up surface-raised space-y-5 p-5 sm:p-7"
      noValidate={false}
    >
      {/* All fields stacked first */}
      ...
      {/* Primary CTA — below the whole form */}
      <Button
        type="submit"
        disabled={pending}
        size="lg"
