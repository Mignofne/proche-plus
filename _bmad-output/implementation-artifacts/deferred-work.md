...
- source_spec: `_bmad-output/implementation-artifacts/spec-multi-exercices-visite-post-outcome.md`
  summary: CAP-4 côté pro — timeline/log listant tous les outcomes d’une session (aidant historique déjà livré via visitCheckInId)
  evidence: Lien `ExerciseAttempt.visitCheckInId` + UI `/aidant/visites` livrés ; reste la surface pro.
